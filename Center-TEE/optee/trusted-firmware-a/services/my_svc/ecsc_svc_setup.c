...
/* ether communication security check runtime service */
...
/* Setup ECSC Service, OEN = 10 */
static int32_t ecsc_svc_setup(void)
{
	int ret = 0;
...
/* ECSC Service SMC Handler */
/* x1: tee_type, x2: address_start, x3: length */
static uintptr_t ecsc_svc_smc_handler(uint32_t smc_fid,
			     u_register_t x1,
			     u_register_t x2,
			     u_register_t x3,
			     u_register_t x4,
			     ...
			     u_register_t flags)
{
	...
	switch (smc_fid)
	{
		...
		default:
			VERBOSE("Unimplemented ECSC Service Call: 0x%x \n", smc_fid);
			SMC_RET1(handle, SMC_UNK);
	}
}

/* Register ECSC Service Calls as runtime service */
DECLARE_RT_SVC(
		ecsc_svc,
		OEN_ECSC_START,
		OEN_ECSC_END,

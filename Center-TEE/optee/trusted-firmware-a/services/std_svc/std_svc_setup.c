			     ...
			     u_register_t flags)
{
	...
	if (is_psci_fid(smc_fid)) {
		VERBOSE("standard service belongs to psci smc\n");
		uint64_t ret;
		...
		VERBOSE("invoke psci smc handler\n");
		...
		VERBOSE("return std_svc smc ret\n");
		SMC_RET1(handle, ret);
	}
	...
	VERBOSE("execute/return arm_std_svc by smc_fid\n");
	switch (smc_fid) {

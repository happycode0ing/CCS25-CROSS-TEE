...
static void setup_secure_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	...
	VERBOSE("Successfully finish setup secure context\n");
}
...
static void setup_ns_context(cpu_context_t *ctx, const struct entry_point_info *ep)
{
	...
	VERBOSE("Successfully finish setup non-secure context\n");
}
...
static void setup_context_common(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	...
	VERBOSE("Successfully finish common initialization\n");
}
...
void cm_setup_context(cpu_context_t *ctx, const entry_point_info_t *ep)
{
	 ...
	 */
	VERBOSE("Perform initializations that are common to all security states\n");
...
#endif
	case NON_SECURE:
		VERBOSE("setup non-secure context\n");
	...
	VERBOSE("Successfully finish all context setup\n");
}
...
{
	cpu_context_t *ctx;
	VERBOSE("get correct context\n");
	ctx = cm_get_context(GET_SECURITY_STATE(ep->h.attr));
	VERBOSE("setup/initialize my context\n");
	cm_setup_context(ctx, ep);
	VERBOSE("Successfully finish all context initialization\n");
}
...
{
	VERBOSE("Check&Restore related registers and context\n");
#if IMAGE_BL31
	/*
	...
	VERBOSE("Successfully prepared to exit from el3 to ns\n");
#endif /* (CTX_INCLUDE_EL2_REGS && IMAGE_BL31) */
}

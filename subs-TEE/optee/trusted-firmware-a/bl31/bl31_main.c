...
void bl31_main(void)
{
	...
	runtime_svc_init();
	INFO("Runtime service initialization finished\n");
	
	/* Initializing secure-device driver: can driver */
	INFO("BL31: Initializing CAN driver\n");
	// can_driver_init();
	INFO("CAN driver initialization finished\n");
	 ...
	 */
	VERBOSE("Prepare to enter the next EL\n");
	 ...
	 * from BL31
	 */
	VERBOSE("Perform any platform specific runtime setup prior to cold boot exit from BL31\n");
	bl31_plat_runtime_setup();

#if ENABLE_RUNTIME_INSTRUMENTATION
	VERBOSE("ENABLE_RUNTIME_INSTRUMENTATION is true\n");
	VERBOSE("flush all consoles for current state\n");
	console_flush();
	VERBOSE("capture&store timestamp\n");
	PMF_CAPTURE_TIMESTAMP(bl_svc, BL31_EXIT, PMF_CACHE_MAINT);
#endif
	VERBOSE("flush all consoles for current state\n");
	console_flush();
	VERBOSE("switch console state into runtime\n");
...
void __init bl31_prepare_next_image_entry(void)
{
	...
	VERBOSE("Determine which image to execute next\n");
	image_type = bl31_get_next_image_type();

	/* Program EL3 registers to enable entry into the next EL */
	VERBOSE("Program EL3 registers to enable entry into the next EL\n");
	...
	*/
	if (image_type == NON_SECURE) {
		VERBOSE("image type is NON_SECURE\n");
		cm_prepare_el3_exit_ns();
	} else {
		VERBOSE("image type is SECURE\n");
		cm_prepare_el3_exit(image_type);
	}
	VERBOSE("Successfully prepare bl31 next image entry\n");
}

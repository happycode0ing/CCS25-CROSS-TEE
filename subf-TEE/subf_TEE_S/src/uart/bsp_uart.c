...
void Debug_UART4_Init(void)
{
    ...
    err = R_SCI_UART_CallbackSet(&g_uart4_ctrl, debug_uart4_callback, NULL, &g_uart_callback_args);
...
int _write(int fd, char *pBuffer, int size)
{
   (void)fd;
   fsp_err_t err;
   err = R_SCI_UART_Write(&g_uart4_ctrl, (uint8_t *)pBuffer, (uint32_t)size);
...
{
    (void)fd;
    R_SCI_UART_Read(&g_uart4_ctrl, (uint8_t *)pBuffer, (uint32_t)size);

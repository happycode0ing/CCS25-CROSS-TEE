...
static StaticTask_t net_thread_memory;
                ...
                static uint8_t net_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t net_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.net_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void net_thread_create(void)
{
    ...
    net_thread = xTaskCreateStatic (
                    ...
                    #endif
                                    net_thread_func,
                                    (const char*) "Net Thread", 1024 / 4, // In words, not bytes

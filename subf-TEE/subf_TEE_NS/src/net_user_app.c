...
int vUDPSend(const char *pcIPAddress, char *pcBufferToTransmit, const size_t xTotalLengthToSend)
{
    ...
    for( ; ulCount < 20; ulCount++)
    {
        /* Create the string that is sent. */
        sprintf( cString, "%s %lu", pcBufferToTransmit, ulCount );
        ...
        int xret = FreeRTOS_sendto( xSocket, cString, strlen( cString ), 0, &xDestinationAddress, sizeof( xDestinationAddress ) );
        if(xret > 0)
        {
            printf("send: %s\r\n", cString);
            ret = 0;

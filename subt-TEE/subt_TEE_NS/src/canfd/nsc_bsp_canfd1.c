 ...
 *      Author: 27951
 */
#include "nsc_bsp_canfd1.h"

can_callback_args_t canfd1_callback_args;

/* CAN 初始化函数 */
void CANFD1_Init(void)
{
    fsp_err_t err = FSP_GUARD_g_canfd1_R_CANFD_Open();

    err = FSP_GUARD_g_canfd1_R_CANFD_CallbackSet(canfd1_callback, NULL, &canfd1_callback_args);
...
void CAN1_Operation(void)
{
    ...
    /* 通过 mail box #0 传输数据 */
    err = FSP_GUARD_g_canfd1_R_CANFD_Write(CAN_MAILBOX_NUMBER_0, &canfd1_tx_frame);
